...
then
  ...
  echo "Input 8 should be the hand tested"
  exit
fi
...
if [ ! -f ${output_dir}/"3dMEMA_${output_prefix}_${hand}.nii.gz" ]
then

    run3dMEMA="3dMEMA -prefix ${output_dir}/3dMEMA_${output_prefix}_${hand}"
    run3dMEMA="${run3dMEMA} -set ${hand}"
  ...
  echo "** ALREADY RUN: ${output_prefix}_${hand} **"
fi
